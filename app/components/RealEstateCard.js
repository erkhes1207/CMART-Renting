...
import data from "../dummyData/RealEstateCardDetail";
    ...
    <>
      {data.map((item, index) => (
        <div
          ...
          onClick={() => router.push(`/real-estate/${item.id}`)}
        >
          ...
          <div className='flex justify-between items-start w-full mt-2 text-md'>
            ...
            <div className='flex w-1/5 justify-end items-center gap-1'>
              ...
              <span className='text-sm font-light'>{item.rate}</span>
            </div>
          </div>
          ...
          <div className='flex gap-1 text-sm'>
            ...
            <span className='font-light'>{item.price} сая</span>
          </div>
        </div>
      ))}
    </>
